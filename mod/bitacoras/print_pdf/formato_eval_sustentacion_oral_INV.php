...
class formato_eval_sustentacion_oral_INV extends FPDF {
...
// Número de página
    ...
    function datos($guid_ev) {
        $evaluacion = new ElggEvalSustentacionOral_inv($guid_ev);

        $this->SetFillColor(181, 181, 181);
        $this->SetTextColor(0, 0, 0);
        $this->SetFont('Arial', 'B', 12);

        $this->MultiCell(190, 6, utf8_decode("VALORACIÓN DE LA SUSTENTACIÓN ORAL DE LA INVESTIGACIÓN (Categoria Investigación)"), 1, 'C', 'true');
        ...
        $this->Ln();
        $this->SetFont('Arial', '', 11);
        $this->Cell(190, 6, utf8_decode($evaluacion->categoriaS), 1);
        ...
        $this->Ln();
        $this->SetFont('Arial', 'B', 12);
        $this->Cell(190, 6, utf8_decode("Apropiación de la investigación y la innovación (Máximo 20 puntos)"), 1, 0, 'C', 'true');
        ...
        //
        $this->Ln();
        $this->MultiCell(115, 6, utf8_decode("Presenta con claridad la pregunta, el problema de investigación y la ruta de indagación recorrida para llegar a los resultados (Máximo 5 puntos)"), 1);
        $this->SetY(88);
        $this->SetX(125);
        $this->SetFont('Arial', 'B', 12);
        ...
        $this->SetFont('Arial', '', 10);
        $this->Ln();
        $this->MultiCell(115, 6, utf8_decode("Explica y argumenta los fundamentos conceptuales que direccionan su investigación y son la base de la innovación (Máximo 5 puntos)"), 1);
        $this->SetY(100);
        $this->SetX(125);
        $this->SetFont('Arial', 'B', 12);
        ...
        $this->SetFont('Arial', '', 10);
        $this->Ln();
        $this->MultiCell(115, 6, utf8_decode("Muestra el proceso metodológico y las evidencias del desarrollo de la investigación que llevaron a obtener la innovación (Máximo 5 puntos)"), 1);
        $this->SetY(112);
        $this->SetX(125);
        $this->SetFont('Arial', 'B', 12);
...
//
        ...
        $this->Ln();
        $this->SetFont('Arial', 'B', 12);
        $this->Cell(190, 6, utf8_decode("Stand o Puesto (máximo 4 puntos)"), 1, 0, 'C', 'true');
        ...
        //
        $this->Ln();
        $this->MultiCell(115, 6, utf8_decode("Hace evidente el diseño investigativo así como el uso de los recursos de propagación y apropiación (Máximo 4 puntos)"), 1);
        $this->SetY(178);
        $this->SetX(125);
        $this->SetFont('Arial', 'B', 12);
