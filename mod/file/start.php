...
function file_init() {
    ...
    elgg_register_action("file/upload", "$action_path/upload.php", "logged_in");
    elgg_register_action("file/delete", "$action_path/delete.php", "logged_in");
    // temporary - see #2010
    elgg_register_action("file/download", "$action_path/download.php", "logged_in");
...
function file_url_override($entity) {
    ...
    //Si la entidad es una red tematica, al hacer clic en el icono del archivo lo redirecciona a la vista del archivo ubicada en el plugin de Redes Tematicas
    $entidad = get_entity($entity->container_guid);

    if ($entidad->getSubtype() == "red_tematica") {
        return "redes_tematicas/archivos/" . $entity->container_guid . "/view/" . $entity->guid;
    }
    //Si la entidad es un grupo de Investigacion,  lo redirecciona a la vista del archivo del Grupo
    else if ($entidad->getSubtype() == "grupo_investigacion") {
        return "grupo_investigacion/archivos/" . $entity->container_guid . "/view/" . $entity->guid;
    }
    //Si la entidad es una feria,  lo redirecciona a la vista del archivo del Grupo
    else if ($entidad->getSubtype() == "feria") {
        return "feria/archivos/" . $entity->container_guid . "/view/" . $entity->guid;
    }
    //Si la entidad es un grupo de Investigacion,  lo redirecciona a la vista del archivo del Grupo
    else if ($entidad->getSubtype() == "institucion") {
        return "instituciones/archivos/" . $entity->container_guid . "/view/" . $entity->guid;
    } else {
        return "file/download/$entity->guid";
