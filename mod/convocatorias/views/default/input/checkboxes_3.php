...
if (count($vars['options']) > 0) {
    ...
    //echo "<table width='80%' class=\"$class\" $id>";
    foreach ($vars['options'] as $label => $value) {
        ...
        $var = explode("/", $label);
        $$var2=explode("/", $value);
        $url = "investigaciones/ver/$var2[0]";
        $params = array(
            'text' => elgg_view('output/longtext', array('value' => $var[0])),
            'href' => $url,
            'is_trusted' => true,
        );
        $title_link = elgg_view('output/url', $params);

        echo "<tr><td class='nombre'>$title_link</td><td class='nombre'>$var[1]</td><td class='nombre'>$var[2]</td><td class='nombre'>$var[3]</td><td>$input</td></tr>";
//        echo "<input type='hidden' name'id_linea' value=$var[2]>";
    }
    //echo '</table>';
}
