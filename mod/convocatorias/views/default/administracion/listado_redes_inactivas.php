

<?php
/**
 * Vista ajax que permite realizar la paginación del listado de las convocatorias inactivas
 */
elgg_load_js('redes-inactivas');
$offset = get_input('offset');
$ajax = get_input('ajax');
$limit = 10;

$url2 = elgg_get_site_url() . "convocatorias/listar_inactivas";
$content = "";
$lista = "";
if (!$ajax) {
    echo '<div style="display:none;" id="dialog-confirm" title="Confirmación"> ¿Está seguro que desea eliminar la convocatoria?</div>';
    ?>

    <div class="content-coordinacion">
        <div class="titulo-coordinacion">
            <h2>Administración</h2>
        </div>
        <div class="menu-coordinacion">
            <?php echo elgg_view("convocatorias/menu_admin", array()); ?>
        </div>

        <div class="contenido-coordinacion">

            <div class="titulo-coordinacion">
                <h2>Redes Temáticas Inactivas</h2>
            </div>

            <?php
            echo "<div id='paginable' class='lista-coordinacion'>";
            echo elgg_get_list_grupos_inactivos($limit, 0, "red_tematica");
            echo "</div>";
            ?>

            <?php
        } else {
            echo elgg_get_list_grupos_inactivos($limit, $offset, "red_tematica");
        }
        ?>

    </div>