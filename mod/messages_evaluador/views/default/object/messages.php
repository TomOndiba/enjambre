...
if ($message->toId == elgg_get_page_owner_guid()) {
	...
	if ($user) {
		...
		$user_link = elgg_view('output/url', array(
			'href' => "messages_evaluador/compose?send_to=$user->guid",
			'text' => $user->name,
			'is_trusted' => true,
		));
	} else {
		$icon = '';
		$user_link = elgg_echo('messages_evaluador:deleted_sender');
	}
	...
	if ($user) {
		...
		$user_link = elgg_view('output/url', array(
			'href' => "messages_evaluador/compose?send_to=$user->guid",
			'text' => elgg_echo('messages_evaluador:to_user', array($user->name)),
			'is_trusted' => true,
		));
	} else {
		$icon = '';
...
$delete_link = elgg_view("output/confirmlink", array(
						'href' => "action/messages_evaluador/delete?guid=" . $message->getGUID(),
...
$body = <<<HTML
<div class="messages_evaluador-owner">$user_link</div>
<div class="messages_evaluador-subject">$subject_info</div>
<div class="messages_evaluador-timestamp">$timestamp</div>
<div class="messages_evaluador-delete">$delete_link</div>
HTML;
