...
foreach ($listado as $user) {
    ...
    $tabla1.="<div>"
            ...
            . "<ul><li><span><a onclick='confirmarEliminarEstudiante(\"{$url_eliminar}\")'>Eliminar</a></span></li></ul>"
            . "</div>";
    $tabla1.= "</li>";
}
...
if (sizeof($listado) > 0) {
    ...
    $int = "<div class='no-element'style='margin-left:40px; margin-top:10px; color:black'><b>Aun no Existen Estudiantes en la Investigación </b></div>";
}
...
$texto="Seleccione los estudiantes que quiera agregar a la iniciativa de investigación: {$cuaderno->name}";
}
else{
 $url_volver="";
 $texto="Seleccione los estudiantes que quiera agregar a la Investigación: {$cuaderno->name}";   
}
...
<div class='box'>
$url_volver
        <h2 class='title-legend'>Agregar Estudiantes</h2>
<div style="display:none;" id="dialog-confirm-eliminar-estudiante" title="Confirmación"> Está seguro de eliminar el Estudiante de la Investigación?</div>
<div class='busqueda-integrantes'>
    <br><label style='color:black'> $texto</label><br><br>
    <input type="text" class="buscar-integrante" id="cajabusqueda" placeholder="Escriba el nombre del estudiante..."/>
