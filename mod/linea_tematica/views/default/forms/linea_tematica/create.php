...
// only extract these elements.
    ...
    if (empty($vars['entity']->guid)) {
        echo "<h2 class='title-legend'>Registrar Red Temática</h2>";
    } else {
        echo "<h2 class='title-legend'>Editar Red Temática</h2>";
    }
    ?>
    <div>
        <label> Ícono de la Red Temática</label>
        <label class="lbl-button">
            <span>Seleccione aquí la imagen</span><?php echo elgg_view("input/file", array('name' => 'icon', 'id' => 'files', 'onchange' => 'nombre(this.value)')); ?>
        </label>
    </div>
    <output id="list"></output>

    <div>
        <label><?php echo "Nombre de la Red" ?></label><br />
        <?php
    ...
    <div>
        <textarea name='description' style='height:100px;' > <?php echo $entity->description ?></textarea>
    </div>
...
<script>
    ...
    $(document).ready(function() {
        $(".elgg-form-redes-tematicas-register").validate({/*sustituir "formulario" por el id de vuestro formulario*/
            rules: {
            ...
            messages: {
                name: {
                    required: "Es necesario el nombre de la Red Temática para registrarlo",
                   
                },
                description: {
                    required: "Este campo es obligatorio, Haga una breve Descripción",
