...
function discussion_init() {
	...
	elgg_register_action('discussion/save', "$action_base/save.php","logged_in");
	elgg_register_action('discussion/delete', "$action_base/delete.php", "logged_in");
	elgg_register_action('discussion/reply/save', "$action_base/reply/save.php","logged_in");
	elgg_register_action('discussion/reply/delete', "$action_base/reply/delete.php", "logged_in");
...
function discussion_page_handler($page) {
	...
	switch ($page[0]) {
		...
		case 'view':
			...
			break;
                case 'reply':
                        discussion_handle_reply_annotation('reply', $page[1]);
                        break;
		default:
			return false;
	}
...
function discussion_override_topic_url($entity) {
    ...
    $container= get_entity($entity->container_guid);
             if($container->getSubtype()=="red_tematica"){
                 return 'redes_tematicas/discusiones/'.$entity->container_guid."/view/".$entity->guid;
             }
             
             else if($container->getSubtype()=="grupo_investigacion")
             {
                 return 'grupo_investigacion/discusiones/'.$entity->container_guid."/view/".$entity->guid;  
             }
             else if($container->getSubtype()=="institucion")
             {
                 return 'instituciones/discusiones/'.$entity->container_guid."/view/".$entity->guid;  
             }
              else{
