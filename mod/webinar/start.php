<?php
	...
	function webinar_init(){
		...
		elgg_register_action("webinar/join", "$action_base/join.php", "logged_in");
		...
		elgg_register_action("webinar/start2", "$action_base/start2.php");
	...
	function webinar_page_handler($page){
		...
		switch($page_type) {
			...
			case "edit":
				gatekeeper();
				if (isset($page[1]) && is_numeric($page[1])) {
					$params = webinar_get_page_content_edit($page_type, $page[1], $page[2], $page[3]);
				}else{
					return false;
				}
				break;
