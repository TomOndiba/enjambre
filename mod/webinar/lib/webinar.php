 ...
 * @return array
 */
function webinar_get_page_content_edit($action_type, $guid = 0, $guid_inv = 0, $guid_asesoria = 0){
	...
	if ($action_type == 'edit') {
		...
		if (elgg_instanceof($webinar, 'object', 'webinar') && $webinar->canEdit()) {
			...
			$body_vars = webinar_prepare_form_vars($webinar, $guid_inv, $guid_asesoria);
		...
		$body_vars = webinar_prepare_form_vars('', $guid_inv, $guid_asesoria);
		$content = elgg_view_form('webinar/save', array(), $body_vars);
	}
 ...
 * @return array
 */
function webinar_prepare_form_vars($webinar = null, $guid_inv = null, $guid_asesoria = NULL) {
