	...
	*/
	...
	public function deleteRecordingsWithXmlResponseArray($recordingParams) {
		...
		*/
        ...
        function bb_wrap_simplexml_load_file($url){
	...
	public function joinURL( $meetingID, $userName, $PW, $SALT, $URL ) {
        ...
        public function elgg_createWebinarArray(ElggUser $user) {
            ...
            public function createWebinarArray( $username, $meetingID, $welcomeString, $mPW, $aPW, $SALT, $URL, $logoutURL ) {
                    ...
                    $xml = bb_wrap_simplexml_load_file( BigBlueButton::createWebinarURL($username, $meetingID, $aPW, $mPW, $welcomeString, $logoutURL, $SALT, $URL ) );
            ...
            public function createWebinarURL($name, $meetingID, $attendeePW, $moderatorPW, $welcome, $logoutURL, $SALT, $URL ) {
                    ...
                    $params = 'name='.urlencode($name).'&meetingID='.urlencode($meetingID).'&attendeePW='.urlencode($attendeePW).'&moderatorPW='.urlencode($moderatorPW).'&voiceBridge='.$voiceBridge.'&logoutURL='.urlencode($logoutURL).'&record='.urlencode($this->record);
