...
$asesor = elgg_get_relationship(get_entity(elgg_get_logged_in_user_guid()), 'asesor');
...
if (!sizeof($asesor) > 0) {
    ...
    $access_label = elgg_echo('Invitar a la Actividad:');
    $access_input = elgg_view('input/radio', array(
        'name' => 'enviar_a',
        'id' => 'enviar_a',
        'options' => array('0' => 'Grupo de Investigación', '1' => 'Miembros de la Investigacón')
    ));
}
...
if (elgg_is_admin_logged_in()) {
    ...
    $server_url_input = elgg_view('input/hidden', array(
        'name' => 'server_url',
    ...
    $server_salt_input = elgg_view('input/hidden', array(
        'name' => 'server_salt',
    ...
    $admin_pwd_input = elgg_view('input/hidden', array(
        'name' => 'admin_pwd',
    ...
    $user_pwd_input = elgg_view('input/hidden', array(
        'name' => 'user_pwd',
        'id' => 'webinar_user_pwd',
        'value' => $vars['user_pwd']
    ));
    ...
    if ($vars['logout_url']) {
        ...
        $logout_url_input = elgg_view('input/hidden', array(
            'name' => 'logout_url',
...
$investigacion = elgg_view('input/hidden', array('name' => 'investigacion', 'value' => $vars['guid_inv']));
$asesoria = elgg_view('input/hidden', array('name' => 'guid_asesoria', 'value' => $vars['guid_asesoria']));
