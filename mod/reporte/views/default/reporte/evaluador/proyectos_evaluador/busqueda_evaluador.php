<h2 class="titulo-reportes title-legend">Reporte Proyectos por Evaluador</h2>

<?php
/**
 * Vista que imprime la búsqueda del asesor
 */
$input = elgg_view('input/text', array('id' => 'input-usuario', 'autocomplete' => "off", 'placeholder' => 'Digite el nombre del Evaluador'));

$usuario = elgg_get_logged_in_user_entity();
$amigos = elgg_get_evaluadores();
...
<script>
    function verificarEvaluador(guid, nombre) {
        elgg.get('ajax/view/reporte/evaluador/proyectos_evaluador/reporte_proyectos_evaluador', {
            timeout: 30000,
            data: {
                 evaluador: guid
            },
...
?>
    ...
    function llenarDivUsuarios(posiciones) {
        ...
        for (var i = 0; i < posiciones.length; i++) {
                ...
                cadena += "<div class='info-evaluador-form' id='" + posiciones[i] + "' onclick='verificarEvaluador(" + guids[posiciones[i]] + ",\"" + amigos[posiciones[i]] + " \" )'><div class='row'><img src='" + imagenes[posiciones[i]] + "' width='32'/></div><div class='row' style='margin-left:10px;font-weight:700;'>" + amigos[posiciones[i]] + "</div></div>";
