...
$(document).ready(function() {
    ...
    var departamento_s=$('#departamento').val();
    var municipio_s=$('#municipio').val();
    ...
    if( municipio_s ==null || municipio_s=="" )
      cargarMunicipios(getMunicipios("Amazonas"));
        ...
        cargarMunicipios(getMunicipios(departamento_s), municipio_s);

    $('#departamentos').live('change', function() {
        var departamento = $(this).val();

        var municipios = getMunicipios(departamento);
        cargarMunicipios(municipios);
    });

    var municipi = $("#municipios").val();
    var dpto = $("#departamentos").val();

    elgg.get('ajax/view/reporte/comunidad/institucion/listar_instituciones_reporte', {
        timeout: 30000,
        data: {
            municipio: municipi,
            departamento: dpto,
...
function cargarMunicipios(municipios, municipio_s) {

    $('#municipios').empty();
    for (i = 0; i < municipios.length; i++) {
        if (municipios[i] == municipio_s) {
            $("#municipios").append('<option value="' + municipios[i] + '"selected>' + municipios[i] + '</option>');
        }
        else {
...
$(document).ready(function() {
    $("#departamentos").live("change", function(evento) {
        var municipi = $("#municipios").val();
        var dpto = $("#departamentos").val();
    ...
    $("#municipios").live("change", function(evento) {
        var municipi = $("#municipios").val();
        var dpto = $("#departamentos").val();
        var inst = $("#departamentos").val();
