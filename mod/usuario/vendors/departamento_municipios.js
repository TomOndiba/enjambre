...
$(document).ready(function() {
    ...
    var departamento_s=$('#departamento_usuario').val();
    var municipio_s=$('#municipio_usuario').val(); 
    if( municipio_s ==null || municipio_s=="" ){  
      cargarMunicipios2(getMunicipios2("Amazonas"));}
    else{
        cargarMunicipios2(getMunicipios2(departamento_s),municipio_s);}
        
    $('#dpto').change(function() {
        var departamento = $(this).val();
        
        var municipios = getMunicipios2(departamento);
        cargarMunicipios2(municipios);
    });
});
function getMunicipios2(departamento) {
    ...
    switch (departamento) {
        ...
        case "Cundinamarca":
            municipios = new Array("Agua de Dios", "Albán", "Anapoima", "Anolaima", "Arbeláez", "Beltrán", "Bituima", "Bogotá","Bojacá", "Cabrera", "Cachipay", "Cajicá", "Caparrapí", "Cáqueza",
...
function cargarMunicipios2(municipios, municipio_s) {
    
   
    $('#munic').empty();
    for (i = 0; i < municipios.length; i++) {
        
        if(municipios[i]== municipio_s)
        $("#munic").append('<option value="' + municipios[i] + '"selected>' + municipios[i] + '</option>');
        else
            $("#munic").append('<option value="' + municipios[i] + '">' + municipios[i] + '</option>');
    } 
}
