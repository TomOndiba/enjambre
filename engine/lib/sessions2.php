...
function pam_auth_userpass(array $credentials = array()) {
    ...
    error_log($user->password.":pass");
    error_log(generate_user_password_2($user, $credentials['password']).":pass2-->".$credentials['password']);
    if ($user->password == generate_user_password($user, $credentials['password'])||$user->password == generate_user_password_2($user, $credentials['password'])) {
         return true;
...
function login(ElggUser $user, $persistent = false) {
    ...
    $roles = elgg_get_roles_usuario($user->getGUID());

    foreach ($roles as $rol) {
        $_SESSION['roles'][] = $rol['nombre'];
    }
    ...
    $mc = new Memcached();
    $mc->addServer("172.18.21.231", 11211);
    ...
    if ($mc->get($user->getGUID())) {
        $mc->set($user->getGUID(), $notificaciones);
    } else {
        $mc->add($user->getGUID(), $notificaciones, time()-(60*60));
    }
