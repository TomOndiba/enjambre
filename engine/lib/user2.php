...
function user_add_request_friend($user_guid, $friend_guid) {
    $user_guid = (int) $user_guid;
    $friend_guid = (int) $friend_guid;
    if ($user_guid == $friend_guid) {
        return false;
    ...
    return add_entity_relationship($user_guid, "request_friend", $friend_guid);
}
...
function user_remove_request_friend($user_guid, $friend_guid) {
    $user_guid = (int) $user_guid;
    $friend_guid = (int) $friend_guid;
    ...
    remove_entity_relationship($friend_guid, "request_friend", $user_guid);
    return remove_entity_relationship($user_guid, "request_friend", $friend_guid);
}
...
function user_is_request_friend($user_guid, $friend_guid) {
    return check_entity_relationship($user_guid, "request_friend", $friend_guid) !== false;
}
...
function send_new_password_request($user_guid) {
    ...
    if ($user instanceof ElggUser) {
        ...
        return notify_user($user->guid, elgg_get_site_entity()->guid, elgg_echo('email:resetreq:subject'), $email, array(), 'email');
    }
    ...
    if ($user instanceof ElggUser) {
        ...
        if ($saved_code && $saved_code == $conf_code) {
            ...
            if (force_user_password_reset($user_guid, $password)) {
                ...
                return notify_user($user->guid, $CONFIG->site->guid, elgg_echo('email:resetpassword:subject'), $email, array(), 'email');
            }
        }
    }
...
function generate_user_password(ElggUser $user, $password) {
    error_log("esta generando md5");
    return md5($password . $user->salt);
}

function generate_user_password_2(ElggUser $user, $password) {
    error_log("esta generando sha");
...
function validate_username($username) {
    ...
    $result = true;
    return elgg_trigger_plugin_hook('registeruser:validate:username', 'all', array('username' => $username), $result);
}
...
function validate_password($password) {
    ...
    $result = true;
    return elgg_trigger_plugin_hook('registeruser:validate:password', 'all', array('password' => $password), $result);
}
...
function validate_email_address($address) {
    ...
    $result = true;
    return elgg_trigger_plugin_hook('registeruser:validate:email', 'all', array('email' => $address), $result);
}
...
function users_init() {
    ...
    elgg_register_action('friends/add_request', "");
