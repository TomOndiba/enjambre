...
jqcc.extend(jqcc.cometchat,{crvariables:{themename:'hangout',timestamp:'0',currentroom:'0',currentp:'',currentroomcode:'',myid:'0',owner:'0',isModerator:0,cu_uids:[],heartbeatTimer:'',baseUrl:'/enjambre/cometchat/',minHeartbeat:'3000',maxHeartbeat:'12000',fullName:'1',hideEnterExit:'0',messageBeep:'1',heartbeatTime:this.minHeartbeat,heartbeatCount:1,todaysDate:new Date(),todaysDay:new Date().getDate(),clh:'',ulh:'',prepend:0,users:{},usersName:{},initializeRoom:0,password:'',currentroomname:'',armyTime:'0',specialChars:/([^\x00-\x80]+)|([&][#])+/,apiAccess:0,lightboxWindows:'1',newMessages:0,plugins:['chattime','colors','filetransfer','smilies'],cookie_prefix:'cc_',basedata:getURLParameter('basedata'),allowDelete:'1',lastmessagetime:Math.floor(new Date().getTime()),floodControl:'0',calleeAPI:'hangout',moderators:[''],windowCount:0,windows:[],cookiePrefix:'cc_'},getcrAllVariables:function(){return this.crvariables;},getChatroomVars:function(key){if(typeof(this.crvariables[key])!=='undefined')
...
message=messagecurrent;jqcc.post(this.crvariables.baseUrl+"modules/chatrooms/chatrooms.php?action=sendmessage",{message:message,currentroom:currentroom,basedata:basedata,currentroomname:currentroomname},function(data){if(data){if(typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].addChatroomMessage)=="function")
jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].addChatroomMessage('1',message,data,1,Math.floor(new Date().getTime()),'0');if(typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].chatroomScrollDown)=="function")
...
jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].loadMobileChatroom();jqcc.cometchat.setChatroomVars('cu_uids','');jqcc.cometchat.chatroomHeartbeat(1);}else{if(data==2){if(silent!=1){alert('Sorry, you are banned from this chatroom.');if(typeof(jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].loadMobileLobbyReverse)=="function")
jqcc[jqcc.cometchat.getChatroomVars('calleeAPI')].loadMobileLobbyReverse();}}else{alert('Incorrect password. Please try again.');}}}});},chatroom:function(id,name,type,invite,silent){name=urldecode(name);if(this.crvariables.currentroom!=id){this.crvariables.password='';if(invite!=''){this.crvariables.password=invite;}
...
(function($){   
	...
	$.ccchattime = (function () {
		...
		var title = 'Mostrar/ocultar hora';
...
if(date!=jqcc.cometchat.getChatroomVars('todaysDay')){return"<span class=\"cometchat_ts\" "+style+">("+hour+":"+minute+ap+" "+date+type+" "+months[month]+")</span>";}else{return"<span class=\"cometchat_ts\" "+style+">("+hour+":"+minute+ap+")</span>";}},addChatroomMessage:function(id,incomingmessage,incomingid,selfadded,sent,cometservice,fromname){if(typeof(fromname)==='undefined'){fromname='Me';}
if(cometservice=='0'){separator=':  ';if($("#cometchat_message_"+incomingid).length>0){$("#cometchat_message_"+incomingid+' .cometchat_chatboxmessagecontent').html(incomingmessage);}else{sentdata='';if(sent!=null){var ts=new Date(parseInt(sent));sentdata=$[calleeAPI].getTimeDisplay(ts,id);}
...
var name=document.getElementById('name').value;var type=document.getElementById('type').value;var password=document.getElementById('password').value;if(name!=''&&name!=null){name=name.replace(/^\s+|\s+$/g,"");if(type==1&&password==''){alert('Please enter a password');return false;}
